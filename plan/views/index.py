...
def get_schedule_starttime() -> Tuple[datetime.datetime, datetime.datetime]:
    ...
    Counts two dates: * beginning of this week
    * end of next week :return:
    """
...
def daterange(start_date: datetime.datetime, end_date: datetime.datetime,
              skip_weekend: bool = True) -> Generator:
...
def index(request):
    ...
    need_to_vote = Idea.objects.filter(approved__isnull=True) \
        .exclude(votes__user=request.user)
